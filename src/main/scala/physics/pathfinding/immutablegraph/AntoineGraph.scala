...
object AntoineGraph {
  ...
  def apply(quadTree: ShapeQT, radius: Double): (Graph, Seq[(Complex, Complex)], List[Segment]) = {
...
//    val neighboursMap = neighboursMapOneWay.foldLeft(neighboursMapOneWay) {
//      case (map, (v, vs)) =>
//        vs.foldLeft(map) {
//          case (m, z) =>
//            m + (z -> (v +: map.getOrElse(z, List())))
//        }
//    }
    ...
    (new Graph(allVertices, neighboursMap), allEdges, inflatedEdges)
  }
